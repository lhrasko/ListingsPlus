...
- (void)viewDidLoad
{
    ...
    inquiry = [InquiryModel new];
    
    if (editInquiry != nil)
    {
        inquiry.source = editInquiry.source;
        inquiry.date = editInquiry.date;
        inquiry.note = editInquiry.note;
        inquiry.feedback = editInquiry.feedback;
        self.title = @"Edit Inquiry";
    }
    else
    {
        inquiry.date = [NSDate date];
        inquiry.source = @"Buyer";
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cellToCheck.tag == 1){
        ...
        if ([cellToCheck.textLabel.text isEqualToString:inquiry.source]) {
    ...
    if (cellToCheck.tag == 2)
    {
        ...
        cellToCheck.detailTextLabel.text = [formatter stringFromDate: inquiry.date];
    }
    if (cellToCheck.tag == 3)
    {
        cellToCheck.detailTextLabel.text = inquiry.note;
    }
    if (cellToCheck.tag == 4)
    {
        cellToCheck.detailTextLabel.text = inquiry.feedback;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cellToCheck.tag == 1)
    {
        inquiry.source = cellToCheck.textLabel.text;
        [self.tableView reloadData];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"segueNotes"]) {
        ...
        destViewController.activityLog = inquiry;
    }
}
...
- (IBAction)SaveButtonPressed:(id)sender {
    if (editInquiry != nil)
    {
        editInquiry.date = inquiry.date;
        editInquiry.note = inquiry.note;
        editInquiry.feedback = inquiry.feedback;
        editInquiry.source = inquiry.source;
        editInquiry.ModifiedDate = [NSDate date];
    }
    else
    {
        Inquiry *inquiryEntity = (Inquiry *)[NSEntityDescription insertNewObjectForEntityForName:@"Inquiry" inManagedObjectContext:managedObjectContext];
        inquiryEntity.date = inquiry.date;
        inquiryEntity.note = inquiry.note;
        inquiryEntity.feedback = inquiry.feedback;
        inquiryEntity.source = inquiry.source;
